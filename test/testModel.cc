...
namespace
{
  ...
  TEST_F(Canvas,findVariableDefinition)
    {
      model=cminsky().model;
    ...
    TEST_F(Canvas, moveIntoThenOutOfGroup)
      {
        model.reset(new Group);
        model->self=model;
        OperationPtr a(OperationType::exp);
    ...
    TEST_F(Canvas, removeItemFromItsGroup)
      {
        model.reset(new Group);
        model->self=model;
