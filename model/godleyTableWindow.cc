...
namespace minsky
{
  ...
  GodleyTableEditor::ClickType GodleyTableEditor::clickType(double x, double y) const
  {
    ...
    if (r==0)
      {
        if (colLeftMargin[c+1]-x < pulldownHot && c!=int(m_godleyIcon.table.cols())-1) //Disable importStock on Equity column. For ticket 1154
          return importStock;
        return row0;
      }
    if (c==0)
