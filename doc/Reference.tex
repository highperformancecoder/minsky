...
{\em post facto} under the ``Axis properties'' context menu. 
...
ports are labeled $x$ and $y$, referring the the formula $x^{y}$.
...
by the operation axis argument---eg a $3\times2$ rank 2 tensor with
...
\begin{center}
\resizebox{\textwidth}{!}{\includegraphics{images/switch}} {\em
...
The final column in the Table, labeled $A-L-E$, checks whether the
...
or sector modeled in it. The inverted wedge \textifsymbol[ifgeo]{99}
...
to or subtractions from the Equity of the relevant sector---which
...
is attached to the top input on the block, labeled $f$; and the
...
otherwise its scope is determined by the innermost containing group
...
plot. This can expedite plot labeling, as well as setting a default
...
specify arbitrary rotations of objects through the edit menu. 
...
hyperinflation for Austria associated with Russia ceasing its post-WWII
...
It has 4 tabs, which lead you through the import process. The first
tab is for selecting the source, which may be a file, one of the
included examples, or a URL to a remote file (possibly zipped).

Once the file is loaded, the form automatically advances to the CSV
setting tab. For conformant CSV files, only the separator character
and quote are important. Some nonconformant files use an escape
character such as `\' to mark the following character as a literal,
and if using space separated data, you may want to treat successive
separators as a single separator, in which case, select ``Merge
delimiters''. If loading European data, you may need to set the decimal
separator to `,'.
...
Once the CSV settings are chosen, then you can move to the next stage,
which is to choose which columns describe the axes of the data, and
...
The system will try to automatically guess which columns are axes and
which are data. Generally speaking, columns consisting entirely of
numbers are considered data, and the other columns axes. Empty columns
are set to ``ignore''. However, this may not always be correct --- for
example if a numerical code is used for classification, or the
numerical data contains currency specifiers like \$ or $\pounds$. You
can override the automatic classification by selecting the
``Dimension'' drop down in each column.

\subsection{Quick instructions for the Data selection form}
...
\item Select Type for included axis columns. Select one of three types: 
  ...
  \item [{time}] data must refer to date-time data. The format field may
    ...
    (0 or 1 respectively).
  \end{description}
\item The header row can be set by clicking in the left hand margin.
\item The column names can be quickly populated from the header row
  from a context menu.
\item Multiple columns can have their types changed simultaneously by click dragging
  to select, then changing the value on one of the columns.
\item When more than one data column exists, it creates a ``horizontal
  dimension''. This has a name, type and format string, just like a
  regular axis column.
\item The ``unique values'' row shows the number of unique values in
  ...
  into the data cube, and so if the hypercube has more than $2^{64}$
...
month representation, then it is important you select \%d or \%e
...
\subsection{Import settings}

The final tab allows a few more options. The first line allows setting
the name, short and long description of the parameter the data is
being imported into. These could also be changed after the data is
imported, so is largely a convenience option.

The second line has options for the import process.

\begin{description}
  \item[Don't fail on invalid data] If selected, Ravel will simply
    ignore any data it cannot understand --- eg malformed dates etc
  \item[Missing value] --- the default is ``NaN'', which stands for
    {\em not a number}, a special IEEE floating point value that Ravel
    understands to be missing data. You can choose to override it, if
    that suits your purpose --- eg substituting 0 for missing data in
    the file.
  \item[Duplicate keys]
    ...
    key, Ravel throws a ``Duplicate key'' exception. This exception
  ...
  automatically selects counter mode.
\end{description}
...
Key Action'' to sum (or perhaps average for this example).
